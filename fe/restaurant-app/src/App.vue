          ...
          <template v-if="isAdmin">
            <li class="nav-item">
              <a class="nav-link collapsible-toggle" href="#" @click.prevent="roomManagementOpen = !roomManagementOpen">
                <span><i class='bx bx-bed'></i> Quản lý phòng</span>
            ...
            <li class="nav-item">
              <a class="nav-link collapsible-toggle" @click.prevent="servicesAmenitiesOpen = !servicesAmenitiesOpen" href="#">
                <span><i class="bi bi-sliders"></i> Dịch vụ & Tiện nghi</span>
...
.btn-outline-custom { border: 1px solid #ccc; color: #555; padding: 8px 18px; border-radius: 4px; background-color: transparent; text-decoration: none; display: inline-block; transition: all 0.2s ease-in-out; }
...
.sidebar { height: 100vh; background-color: #fff; box-shadow: 2px 0 5px rgba(0, 0, 0, 0.1); padding-top: 20px; position: fixed; width: 250px; z-index: 1000; overflow-y: auto; scrollbar-width: none; }
...
.sidebar .profile .icons i { font-size: 1.2rem; margin: 0 5px; color: #6c757d; cursor: pointer; transition: color 0.3s ease; }
.sidebar .profile .icons i:hover { color: #0a75f7; }
.sidebar .nav-link { color: #0c0c0c; padding: 10px 20px; display: flex; align-items: center; transition: all 0.3s ease; }
.sidebar .nav-link:hover, .sidebar .nav-link.router-link-active { color: #0a75f7; background-color: #f8f9fa; }
...
.sidebar .collapsible-toggle .toggle-icon { transition: transform 0.3s ease; font-size: 0.8rem; margin-right: 0; }
.sidebar .collapsible-toggle .toggle-icon.rotated { transform: rotate(180deg); }
.nav-submenu { list-style: none; padding-left: 0; margin: 0; max-height: 0; overflow: hidden; transition: max-height 0.35s ease-in-out; background-color: #fdfdfd; }
.nav-submenu.open { max-height: 200px; }
.nav-submenu .nav-link { padding-left: 40px; background-color: transparent !important; }
.nav-submenu .nav-link.router-link-active { color: #0a75f7; }
.nav-submenu .nav-link:hover { color: #0a75f7; }
.main-content { margin-left: 250px; padding: 20px; padding-top: 60px; min-height: 100vh; background-color: #f5f7fb; width: calc(100% - 250px); overflow-y: auto; }
.navbar-top { padding: 10px 20px; display: flex; justify-content: flex-end; align-items: center; position: fixed; top: 0; left: 250px; right: 0; z-index: 900; height: 50px; background-color: #f5f7fb; transition: background-color 0.1s ease; }
