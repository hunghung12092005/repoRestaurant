    ...
    <div class="row mb-4 g-3">
      <div class="col-md-4">
        <input
          ...
          type="text"
          class="form-control"
          placeholder="Tìm tên bàn, loại bàn hoặc trạng thái..."
        />
      </div>
      <div class="col-md-3">
      ...
      <table class="table table-bordered table-hover align-middle">
        ...
        <tbody>
          <tr v-for="ban in banHienThi" :key="ban.table_id">
            ...
            <td>{{ ban.type_name || 'Không xác định' }}</td>
            <td class="status">
              <span
                :class="{
                  badge: true,
                  ...
                  'bg-secondary': ban.status === 'Bảo trì'
                }"
              >
                {{ ban.status }}
              </span>
            ...
            <td>{{ ban.capacity }}</td>
            <td>{{ ban.location || 'Chưa xác định' }}</td>
            <td>{{ ban.description || 'Không có' }}</td>
            <td>
              <button
    ...
    <div class="d-flex justify-content-between align-items-center mt-4">
      ...
      <nav>
        <ul class="pagination mb-0">
          ...
          >
            <button class="page-link" @click="trangHienTai = trang">
              {{ trang }}
            </button>
          </li>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <select v-model="banMoi.type_id" class="form-select">
  ...
  setup() {
    ...
    const fetchTables = async () => {
      try {
        ...
        console.log("Phản hồi GET /api/tables:", res.data);
    ...
    // Lấy danh sách loại bàn từ API
    const fetchTableTypes = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/table-types", {
          timeout: 5000,
        });
        console.log("Phản hồi GET /api/table-types:", res.data);
        tableTypes.value = res.data.data || [];
        if (!res.data.data) {
          console.warn("Không tìm thấy dữ liệu loại bàn trong phản hồi");
        }
      } catch (error) {
        let errorMessage = "Lấy dữ liệu loại bàn thất bại";
        if (error.code === "ECONNABORTED") {
          errorMessage += ": Yêu cầu hết thời gian (timeout)";
        } else if (error.response) {
          errorMessage += `: ${error.response.status} - ${error.response.data.message || error.message}`;
        ...
        alert(errorMessage);
        console.error("Lỗi lấy dữ liệu loại bàn:", error);
      }
    };
    ...
    const banLoc = computed(() => {
      return ban.value.filter((b) => {
        const khopTim =
          b.table_name.toLowerCase().includes(tuKhoaTim.value.toLowerCase()) ||
    ...
    const luuBan = async () => {
      ...
      try {
        ...
        if (banMoi.value.type_id === 0) {
          throw new Error("Vui lòng chọn loại bàn");
        }
        ...
        if (!["Trống", "Chờ xác nhận", "Đã đặt", "Bảo trì"].includes(banMoi.value.status)) {
          throw new Error("Trạng thái không hợp lệ");
        }

        console.log("Dữ liệu gửi đi:", banMoi.value);
        ...
        if (banDangSua.value) {
          ...
          console.log("Phản hồi PUT /api/tables:", res.data);
          ...
          console.log("Phản hồi POST /api/tables:", res.data);
