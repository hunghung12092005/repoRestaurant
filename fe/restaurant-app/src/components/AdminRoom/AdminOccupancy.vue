            ...
            <div class="row g-3">
              <div class="col-md-3">
                ...
                <select class="form-select" v-model="booking.room_id">
                  ...
                  <option v-for="room in availableRooms" :key="room.room_id" :value="room.room_id">{{ room.number }}</option>
                </select>
              </div>
              <div class="col-md-3"><label class="form-label">Họ tên</label><input v-model="booking.customer_name" class="form-control" /></div>
              <div class="col-md-3"><label class="form-label">SĐT</label><input v-model="booking.customer_phone" class="form-control" /></div>
              <div class="col-md-3"><label class="form-label">CCCD</label><input v-model="booking.customer_id_number" class="form-control" /></div>
              <div class="col-md-4"><label class="form-label">Email</label><input v-model="booking.customer_email" class="form-control" /></div>
              <div class="col-md-4"><label class="form-label">Ngày nhận</label><input type="date" v-model="booking.check_in_date" class="form-control" /></div>
              <div class="col-md-4"><label class="form-label">Giờ nhận</label><input type="time" v-model="booking.check_in_time" class="form-control" /></div>
              <div class="col-md-4"><label class="form-label">Ngày trả</label><input type="date" v-model="booking.check_out_date" class="form-control" /></div>
              <div class="col-md-4"><label class="form-label">Giờ trả</label><input type="time" v-model="booking.check_out_time" class="form-control" /></div>
              <div class="col-md-4">
  ...
  <div v-if="showPayGroupModal" class="modal fade show d-block" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered">
      <div class="modal-content modal-custom">
        <div class="modal-header modal-header-custom">
          <h5 class="modal-title">Thanh toán nhóm</h5>
          <button type="button" class="btn-close" @click="showPayGroupModal = false"></button>
        </div>
        <div class="modal-body p-4">
          <label class="form-label">Chọn Booking ID:</label>
          <select class="form-select mb-3" v-model="selectedBookingId" @change="loadBookingGroupRooms">
          ...
          <div v-if="bookingGroupRooms.length > 0">
            <div v-for="(room, index) in bookingGroupRooms" :key="room.booking_detail_id" class="border rounded p-3 mb-3 bg-light">
              <h6>Phòng {{ room.room_name }} ({{ room.type_name }})</h6>
              <div v-for="(service, sIndex) in room.services" :key="sIndex" class="d-flex justify-content-between align-items-center">
                <span>{{ service.service_name }} ({{ formatPrice(service.price) }})</span>
                <div class="d-flex align-items-center">
                  <button class="btn btn-sm btn-outline-secondary" @click="service.quantity = Math.max(service.quantity - 1, 0)">-</button>
                  <input type="number" v-model.number="service.quantity" class="form-control form-control-sm text-center mx-2" min="0" />
                  <button class="btn btn-sm btn-outline-secondary" @click="service.quantity++">+</button>
                </div>
              </div>
              <div class="mt-2">
                <label>Phụ thu:</label>
                <input type="number" v-model.number="room.additional_fee" class="form-control form-control-sm" placeholder="0" />
        ...
        <div class="modal-footer modal-footer-custom">
          <button class="btn btn-secondary" @click="showPayGroupModal = false">Hủy</button>
          <button class="btn btn-success" @click="submitPayGroup">Thanh toán tất cả</button>
        </div>
      </div>
    </div>
  </div>
