      ...
      <div class="d-flex flex-grow-1 overflow-hidden">
        ...
        <aside class="bg-light border-end" style="width: 300px; overflow-y: auto;">
          ...
          <ul class="list-group list-group-flush">
            <li v-for="user in users"
              ...
              <span v-if="unreadCounts[user.userId] > 0"
                    class="badge bg-danger rounded-pill">{{ unreadCounts[user.userId] }}</span>
            </li>
          </ul>
        </aside>
        ...
        <section class="flex-grow-1 d-flex flex-column overflow-hidden">
          ...
          <div class="bg-white border-bottom p-3 d-flex align-items-center">
            <img src="https://png.pngtree.com/element_our/20190528/ourlarge/pngtree-blue-sms-message-communication-2-5d-resume-small-icon-image_1185735.jpg"
            ...
            <div v-if="selectedMessages.length">
              <div v-for="(msg, index) in selectedMessages" :key="index"
                ...
                <div class="p-3 rounded-3 shadow-sm"
                  ...
                  <img v-if="msg.file" :src="msg.file" class="img-fluid rounded mt-2" style="max-width: 200px;" />
                </div>
              </div>
            </div>
...
const listenForMessages = () => {
    ...
    //     //console.log(data2);
    //     socketIdUser.value = data2.socketID;
    //     //console.log(socketIdUser.value);
    //     // Kiểm tra xem tin nhắn có phải của người dùng được chọn không
    //     const messageObject = { user: data2.user, message: data2.message };
    //     if (data2.userId == selectedUserId.value) {
    //         if (data2.file) {
    //             messageObject.file = data2.file; // Lưu đường dẫn file vào đối tượng tin nhắn
    //         }

    //         selectedMessages.value.push(messageObject); // Thêm tin nhắn mới vào danh sách
