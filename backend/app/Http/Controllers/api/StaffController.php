...
class StaffController extends Controller
{
    public function index(Request $request)
    {
        ...
        $staff = User::with(['staffProfile', 'staffSchedules'])
            ->whereIn('role', ['manager', 'receptionist', 'housekeeping', 'technician'])
            ->paginate($perPage);
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'role' => 'required|in:manager,receptionist,housekeeping,technician',
            'status' => 'required|in:active,inactive,suspended',
            'permissions' => 'array',
            'permissions.*' => 'in:manage_bookings,manage_staff,manage_reports,manage_cleaning,manage_maintenance',
        ...
        try {
            $user = User::create([
                ...
                'permissions' => json_encode($request->permissions ?? $this->getPermissions($request->role)),
            ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Xóa lịch làm việc tương lai nếu trạng thái là inactive hoặc suspended
            if ($request->status !== 'active') {
                ...
                // Xử lý lịch làm việc
                $existingScheduleIds = $user->staffSchedules->pluck('id')->toArray();
                $newScheduleIds = array_filter(array_column($request->schedules ?? [], 'id'), fn($id) => !is_null($id));

                // Xóa các lịch không còn trong danh sách gửi lên
                StaffSchedule::where('user_id', $user->id)
                    ->whereNotIn('id', $newScheduleIds)
                    ->delete();
