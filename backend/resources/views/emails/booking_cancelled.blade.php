...
<html>
<head>
    <title>Hủy đặt phòng Mã đơn: HXH{{ $booking->booking_id }}</title>
    <style>
...
<body>
    ...
    <h1>Hủy đặt phòng Mã đơn: HXH{{ $booking->booking_id }}</h1>
    <p>Kính gửi {{ $booking->customer->customer_name ?? 'Quý khách' }},</p>
    <p>Đặt phòng của bạn đã bị hủy.</p>
    ...
    @if (!empty($additionalInfo['refund_amount']))
        <p>Số tiền hoàn lại: {{ number_format($additionalInfo['refund_amount'], 0, ',', '.') }} VND</p>
        @if (!empty($additionalInfo['refund_bank']))
            <p>Thông tin hoàn tiền:</p>
            <ul>
                <li>Ngân hàng: {{ $additionalInfo['refund_bank'] }}</li>
                <li>Số tài khoản: {{ $additionalInfo['refund_account_number'] }}</li>
                <li>Tên tài khoản: {{ $additionalInfo['refund_account_name'] }}</li>
            </ul>
        @endif
    @endif
    <p>Chúng tôi rất tiếc vì sự bất tiện này. Nếu bạn có bất kỳ câu hỏi nào, vui lòng liên hệ với chúng tôi.</p>
    <div class="footer">
